import { isRenderableScene, normalizeScene, type TemplateScene } from './template-lesson'

/** Newline-delimited events sent from `/api/lesson` to the player. */
export type LessonEvent =
  | { type: 'meta'; title: string; summary: string }
  | { type: 'scene'; index: number; scene: TemplateScene }
 ...
 * so the player starts on scene one while the model writes scene four. Same
 * mechanism as the whiteboard engine; only the scene type differs.
 */
export class LessonStreamParser {
  ...
  push(chunk: string): LessonEvent[] {
    ...
    while (!this.closed) {
      ...
      let scene: TemplateScene | null = null
      try {
        scene = JSON.parse(found.json) as TemplateScene
      } catch {
        continue
      }
