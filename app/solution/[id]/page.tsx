...
export const metadata: Metadata = {
  title: 'Worked solution — nipsol',
}

/**
 * One worked solution, at its own address.
 *
 ...
 */
export default async function SolutionPage(ctx: PageProps<'/solution/[id]'>) {
  const { id } = await ctx.params
  if (authConfigured() && !(await currentUser())) redirect(`/login?next=/solution/${id}`)

  return <MindmapStudio open={{ kind: 'math', id }} />
}
