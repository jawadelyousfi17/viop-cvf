...
import { buildActs, stationBounds, type Act, type CueSheet } from './mural'
import './demo.css'
  ...
  return (
    <main className="plate bare">
      ...
      <div className="corner right">
        {started ? `${String(sceneIndex + 1).padStart(2, '0')} — ${titles[sceneIndex]}` : ''}
      </div>

      <div className={`card${started ? ' gone' : ''}`}>
        <p className="micro" style={{ letterSpacing: '0.32em' }}>
          drawn onto one wall · 9 minutes
        </p>
        <h1>What Docker actually does</h1>
        <p>
          Seventeen stations on a single canvas, drawn in time with the narration. The camera
          travels as the story does — and when it ends, it pulls back to show you the whole
          wall. Pause any time and the drawings are real: pan, zoom, pick them up.
        </p>
      ...
      <div className={`runner${idle && playing ? ' away' : ''}`}>
        ...
        <span style={{ minWidth: 110, textAlign: 'right' }}>
          station {String(sceneIndex + 1).padStart(2, '0')}/17
        </span>
      </div>
